...
class Query(ObjectType):
    ...
    track = Field(Track, id=ID(required=True))
    album = Field(Album, id=ID(required=True))
    person = Field(Person, id=ID(required=True))
    user = Field(User, id=ID(required=True))
    label = Field(Label, id=ID(required=True))
    stat = Field(Stat, id=ID(required=True))
    file = Field(File, id=ID(required=True))
    genre = Field(Genre, id=ID(required=True))

    def resolve_user(self, info: dict, user_id: int) -> User:
        """Logic to resolve a user by ID."""
